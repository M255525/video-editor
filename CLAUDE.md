...
## 匯出閃爍／黑屏修正（2026-08-19 新增，同一天緊接在時長修正之後）

使用者回報「輸出影片為什麼會閃爍 還有黑屏」。根因在快速匯出（`fastExport`）逐幀呼叫的 `seekVisualsTo(t)`，有兩個獨立問題：

1. **黑屏**：片段剛匯入、還沒在編輯畫面播放/拖曳過時，`VE.ensureClipEl(clip)` 建立的 `<video>` 元素 `readyState` 還是 0（`HAVE_NOTHING`）。舊版邏輯的判斷式是 `if (el.readyState >= 1 && ...)` 才會等待 seek，**readyState 不足 1 時完全不等、直接放行**，`drawFrame` 就會畫到完全沒資料的空畫面——最常出現在匯出片頭（因為片頭的片段最可能是「剛匯入還沒被存取過」的狀態）。
2. **閃爍**：`seeked` 事件只保證「跳轉這個動作完成」，不保證該時間點的畫面已經解碼完成、可以正確被 `drawImage` 取用。快速匯出是程式在極短時間內連續大量 seek（比真人手動拖曳快很多），很容易畫到「還沒解碼完的舊畫面」造成閃爍/殘影。

**修法**（`export.js` 的 `seekVisualsTo`）：
- `readyState < 1` 時先等 `loadedmetadata`（2000ms 逾時保底）才繼續，不再無條件放行。
- `seeked` 觸發後，若瀏覽器支援 `requestVideoFrameCallback`，再多等一次「畫面真的呈現」的確認（比 `seeked` 更可靠，rVFC 保證的是「有新的一幀被合成呈現」）；不支援則退回 `requestAnimationFrame`。
- **重要踩坑（開發過程中自己踩到、已修正）**：第一版加了 rVFC 等待卻忘記加逾時保底，結果整個匯出卡死在「準備編碼器…」／「編碼音訊…」不動——因為 rVFC 的回呼是「有新的一幀呈現時才觸發」，如果當下位置本來就已經是目標幀（沒有發生新的呈現事件），回呼可能永遠不會來。**現在每一層等待（loadedmetadata／seeked／requestVideoFrameCallback）都各自有獨立的逾時保底**，任何一層卡住都不會拖垮整個匯出流程，這是這次修正裡最重要的設計原則。
- **已用 Playwright 端對端嚴格驗證**：合成一段「每 0.5 秒切換純色（紅/綠/藍/黃）」的測試影片，**完全不經過編輯畫面播放/預覽**（模擬「剛匯入就直接匯出」的最容易觸發黑屏的情境）、直接觸發真正的匯出流程，攔截 `URL.createObjectURL` 取得實際輸出的 MP4 二進位內容存檔，再用 `ffmpeg` 抽取影格、逐格量測畫面色彩。結果：**第一格就是純紅色（不是黑屏），全程 26 個取樣點色彩皆乾淨純粹、精確對應應有的色彩序列，沒有任何黑幀、殘影或中間過渡的髒幀**——這比單純「看它有沒有跑完」更嚴謹，是真的驗證了輸出內容本身的正確性。根目錄版與 `mrvideo_s` 同步套用同一套修正。
