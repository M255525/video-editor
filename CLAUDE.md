...
## 編輯預覽「按暫停鍵時會出現黑屏／往前移動時也會黑屏」修正（2026-08-19 新增；同一天內修了兩次，第二次才是真正定案）

跟上面「匯出閃爍／黑屏」是**兩個不同的 bug**——那個發生在匯出（`export.js` 的 `fastExport`），這個發生在**編輯畫面的即時預覽播放**（`preview.js` 的 `syncMedia`/`VE.pause`/`VE.seek`），成因也完全不同，不要混為一談。

**第一次修正（後來證實不夠完整）**：根因診斷為 `syncMedia(t, isPlaying)` 的容差設計——播放中用寬鬆容差 `tol=0.25`、暫停時改嚴格容差 `tol=0.03`，播放中的自然漂移量常剛好落在兩者之間，暫停瞬間觸發一次新的非同步校正 seek，緊接著的 `VE.drawFrame()` 畫在 seek 完成之前抓到黑畫面，且播放時鐘已停、之後不會再有 `requestAnimationFrame` 補畫。當時比照 `VE.seek()` 既有做法，在 `VE.pause()` 也加上「seek 完成後 `setTimeout(...,120)` 延遲補畫一次」。**這個判斷方向是對的，但做法不夠**——使用者部署後回報「按暫停鍵還是會有黑屏 或是往前移動時也會出現」，代表：① 120ms 是憑感覺猜的固定值，不是真的等到畫面就緒，跟匯出流程踩過的坑一樣（冷片段/較大檔案/往前跳較遠時，seek 真正完成可能遠超過 120ms）；② `VE.seek()` 本身（拖曳/點擊時間軸移動播放頭）也有同一套猜測性延遲，同樣可能不夠。

**第二次修正（真正定案）**：連本帶利重新設計 `syncMedia` 的 seek 等待機制，這次挖出**兩個疊加的根因**：

1. **固定延遲改成真正等待 `seeked` 事件**：`syncMedia(t, isPlaying, waits)` 新增第三個選填參數，非播放狀態呼叫時傳入一個陣列，每次觸發 seek 就往裡面塞一個「監聽 `seeked` 事件才 resolve」的 Promise（4000ms 逾時保底，跟匯出流程 `seekVisualsTo()` 用同一個值，只在真的異常時才會用到）。`VE.pause()`／`VE.seek()` 改成 `Promise.all(waits).then(繪圖)`，不再是「猜一個時間應該夠了」。加上 `drawGen` 世代編號：連續觸發多次等待時，只有最新一次完成才真的補畫，避免舊的、比較慢回來的那次把新畫面蓋回過期內容。
2. **更關鍵的獨立根因（在重新設計時意外挖出，不是原本假設的一部分）**：`syncMedia` 原本的觸發條件是 `if (!el.seeking && Math.abs(el.currentTime - st) > tol)`——**只要 `<video>` 元素正在 seek 中，這次呼叫完全不處理，連 `el.currentTime` 都不會更新到最新目標**。使用者拖曳時間軸／快速連續操作播放頭時，`VE.seek()` 會連續呼叫多次，第一個 seek 還沒完成，後面幾次呼叫全部被這個條件擋掉——最後畫面卡在中途某個目標，跟真正的播放頭位置對不上（**這正是「往前移動時也會出現黑屏」的根因，不是延遲時間不夠，是新目標根本沒送出去**）。修法：改用 `entry.seekTarget` 追蹤「目前真正在等待的目標」（seeking 期間 `el.currentTime` 不可靠，不能拿來當比較基準），只要新目標跟這個追蹤值差距超過容差就重新指定 `el.currentTime`——瀏覽器原生支援「seek 進行中再次設定 currentTime」會直接重新導向到最新目標，不需要等前一個 seek 先完成。

**已用 Playwright 端對端驗證三種情境**（因為真實播放時鐘 `requestAnimationFrame` 在無焦點/背景分頁下會被嚴重節流，難以穩定重現「播放中漂移」，改用直接操作 `<video>` 元素模擬狀態的方式）：
- **情境A（暫停時的漂移）**：手動把 video 元素 seek 到紅色段並等待完成，模擬「播放頭已到綠色段但元素還停在紅色段」的漂移狀態，呼叫 `VE.pause()`——暫停瞬間讀到黑色，延遲補畫後正確變綠色。
- **情境B（快速連續 seek，模擬拖曳時間軸）**：連續呼叫 `VE.seek(0.3)→VE.seek(3.0)→VE.seek(5.5)→VE.seek(9.0)` 不等待中間完成——**第一次修正版本這裡會卡在錯誤顏色**（`el.seeking` guard 擋掉中間所有更新，最終畫面停在最早那次 seek 的目標，跟播放頭真正位置對不上），**第二次修正後正確收斂到最終目標（黃色）**。
- **情境C（冷片段往前跳）**：先丟棄 video 元素模擬「完全沒被瀏覽器碰過」的冷啟動狀態，再直接 seek 到片段中後段——正確等到畫面就緒才補畫，不會提早顯示黑屏。

三種情境根目錄版與 `mrvideo_s` 皆各自驗證通過。**教訓**：使用者第一次回報「按暫停鍵時會出現黑屏」時只描述了一種操作，我當時只驗證了「暫停」這一條路徑就以為修好了；使用者接著回報「還是會有黑屏，往前移動時也會」才逼出更完整的重新設計——**同一份 `syncMedia` 邏輯被 `VE.pause()`／`VE.seek()`／`step()` 共用，修正畫面正確性問題時要連帶檢查所有呼叫路徑，不能只驗證回報中明確提到的那一個操作**，這跟先前「匯出黑屏」教訓的「測試素材要夠真實」是同一類「驗證範圍不夠完整」的坑，但這次是「操作路徑覆蓋不夠完整」。
