...
## 編輯預覽「按暫停鍵時會出現黑屏」修正（2026-08-19 新增，與根目錄版本同一套邏輯）

跟上面「匯出閃爍／黑屏」是不同的 bug，發生在編輯畫面即時預覽播放（`preview.js` 的 `VE.pause()`），不是匯出流程。根因：`syncMedia(t, isPlaying)` 播放中用寬鬆容差（0.25s）、暫停時改嚴格容差（0.03s），`<video>` 元素播放中的自然漂移量常剛好落在這兩者之間，暫停瞬間觸發一次新的非同步校正 seek，`VE.pause()` 緊接著呼叫的 `VE.drawFrame()` 畫在 seek 完成之前，抓到黑畫面，且播放時鐘已停、之後不會再有 `requestAnimationFrame` 補畫，畫面卡住。**修法**：比照 `VE.seek()` 早就有的「seek 完成後 `setTimeout(...,120)` 延遲補畫一次」做法，同樣套用到 `VE.pause()`（`redrawTimer` 變數宣告移到 `VE.pause` 之前，兩函式共用）。已用 Playwright 端對端驗證（直接操作 video 元素模擬播放中漂移狀態，因為真實 `requestAnimationFrame` 播放時鐘在背景分頁下會被嚴重節流、難以穩定重現）：暫停瞬間 canvas 讀到黑色，120ms 延遲補畫後正確變成目標畫面顏色。詳細成因與驗證方式見 `../CLAUDE.md` 同名章節，這裡不重複。
