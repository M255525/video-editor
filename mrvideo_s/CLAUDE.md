...
## 匯出閃爍／黑屏修正（2026-08-19 新增，與根目錄版本同一套邏輯）

跟根目錄版本一樣修正了 `export.js` 的 `seekVisualsTo()`（快速匯出逐幀 seek 邏輯）：片段剛匯入還沒讀過就直接匯出會黑屏、`seeked` 事件不保證畫面已解碼完成會閃爍，兩者都已修正，且每層等待都有獨立逾時保底避免卡死。詳細設計與 Playwright 像素級驗證方式見 `../CLAUDE.md` 同名章節。已重新用同一套端對端流程（合成彩色測試影片、完全不經預覽直接匯出）驗證這裡的匯出流程能正常進度不卡死。
