...
## 匯出閃爍／黑屏修正（2026-08-19 新增，與根目錄版本同一套邏輯，同一天內修了兩次）

跟根目錄版本一樣修正了 `export.js` 的 `seekVisualsTo()`：真正且唯一的黑屏成因是片段剛匯入還沒讀過就直接匯出，已修正（先等 `loadedmetadata` 才 seek）。**中途一度加了 `requestVideoFrameCallback` 想解決閃爍，後來用真實長 GOP 測試影片重新驗證證實這個方向是錯的且有害**（rVFC 在同一元素連續大量呼叫下不穩定、拖慢匯出超過一個數量級；`seeked` 本身在此情境下已經可靠，不需要額外驗證）——**已拿掉 rVFC，只留 loadedmetadata＋seeked 兩層**。詳細的踩坑過程、兩次測試的差異（合成近全關鍵影格素材 vs 真實長 GOP 素材）見 `../CLAUDE.md` 同名章節，這裡不重複。已用同一套嚴謹驗證（真實長 GOP 測試影片、ffmpeg 逐格量測色彩）重新確認這裡零黑幀零錯幀。
